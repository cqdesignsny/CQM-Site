...
export async function POST(request: NextRequest) {
  try {
    ...
    const subject = isEs
      ...
      : isFr
        ? `Résultats de l'Évaluation Marketing — ${body.senderName}`
      ...
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
        <div style="text-align: center; margin-bottom: 32px;">
          <h1 style="font-size: 24px; color: #111; margin: 0;">
            ${isEs ? "Resultados de Evaluación" : isFr ? "Résultats de l'Évaluation" : "Assessment Results"}
          </h1>
          <p style="color: #666; font-size: 14px; margin-top: 8px;">
            ${isEs ? "de" : isFr ? "de" : "from"} ${siteConfig.name}
          </p>
        </div>
        ...
        <div style="background: #f8f9fa; border-radius: 12px; padding: 24px; text-align: center; margin-bottom: 24px;">
          <p style="color: #888; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; margin: 0 0 8px;">
            ${isEs ? "Puntuación General" : isFr ? "Score Global" : "Overall Score"}
          </p>
        ...
        <p style="color: #555; font-size: 14px; line-height: 1.6;">
          ${isEs
            ...
            : isFr
              ? `${body.senderName} a complété une évaluation marketing et souhaite partager les résultats avec vous. Cliquez ci-dessous pour voir le détail complet et les recommandations.`
        ...
        <div style="text-align: center; margin: 32px 0;">
          <a href="${resultsUrl}" style="display: inline-block; background: #dc2626; color: white; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; font-size: 14px;">
            ${isEs ? "Ver Resultados Completos" : isFr ? "Voir les résultats complets" : "View Full Results"}
          </a>
        </div>
